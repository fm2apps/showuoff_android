...
public class PhotoEditorSDK implements MultiTouchListener.OnMultiTouchListener {
    ...
    public void addImage(String desiredImage)
    {
        ...
        Picasso.with(context).load(Constants.FILTER_IMAGE_URL+desiredImage).into(imageView);
    ...
    public void addGif(String desiredImage)
    {
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View imageRootVieww = inflater.inflate(R.layout.photo_editor_sdk_gif_item_list, null);
        SimpleDraweeView imageView = (SimpleDraweeView) imageRootVieww.findViewById(R.id.profile_imagee);

        Uri loader = Uri.parse(Constants.FILTER_IMAGE_URL+desiredImage);
        DraweeController controllerOne = Fresco.newDraweeControllerBuilder()
                .setUri(loader)
                .setAutoPlayAnimations(true)
                .build();
        imageView.setController(controllerOne);
        ...
        imageRootVieww.setOnTouchListener(multiTouchListener);
        RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
                ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        params.addRule(RelativeLayout.CENTER_IN_PARENT, RelativeLayout.TRUE);

        parentView.addView(imageRootVieww, params);
        addedViews.add(imageRootVieww);
        if (onPhotoEditorSDKListener != null)
            onPhotoEditorSDKListener.onAddViewListener(ViewType.IMAGE, addedViews.size());
    }
        ...
        if (isSDCARDMounted()) {
            ...
            try
            {
                ...
                if (parentView != null)
                {
                    ...
                    parentView.getDrawingCache().compress(Bitmap.CompressFormat.JPEG, 65, out);
                }
                out.flush();
                out.close();
            } catch (Exception e) {
